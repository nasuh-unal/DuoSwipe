...
class GoogleAuthUiClient(
    ...
    suspend fun signInWithIntent(intent: Intent): SignInResult {
        val credental = oneTapClient.getSignInCredentialFromIntent(intent)
        val googleIdToken = credental.googleIdToken
    ...
    fun getSignInUser(): UserData? = auth.currentUser?.run {
        UserData(
            userId = uid, username = displayName, profilePictureId = photoUrl?.toString()
        )
    }
